...
streznik.get('/', function(zahteva, odgovor) {
  if (!zahteva.session.currentUser){
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function(napaka1, polja, datoteke){
    ...
    strankaIzRacuna(idRacuna, function(stranke){
      var stranka = stranke[0];
      pesmiIzRacuna(idRacuna, function(pesmi){
        odgovor.setHeader('content-type', 'text/xml');
        odgovor.render('eslog', {vizualiziraj: true, postavkeRacuna: pesmi, narocnik: stranka});
      });
    });
  });
});
  ...
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      vrniStranke(function(napaka, vrstice){
        if (napaka) return;
        var stranka = vrstice[parseInt(zahteva.session.currentUser) - 1];
        odgovor.setHeader('content-type', 'text/xml');
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    vrniStranke(function(napaka3, stranke){
      vrniRacune(function(napaka4, racuni){
        if (napaka1 || napaka2) odgovor.render("prijava", {sporocilo: "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.", 
                                                            seznamStrank: stranke, 
                                                            seznamRacunov: racuni});
        else odgovor.render("prijava", {sporocilo: "Stranka je bila uspešno registrirana.", 
                                        seznamStrank: stranke, 
                                        seznamRacunov: racuni});
...
streznik.post('/odjava', function(zahteva, odgovor) {
    delete zahteva.session.currentUser;
    odgovor.redirect('/prijava') 
})
